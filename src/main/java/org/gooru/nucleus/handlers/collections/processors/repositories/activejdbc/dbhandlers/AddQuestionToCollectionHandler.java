...
class AddQuestionToCollectionHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    ...
    return AuthorizerBuilder.buildAddContentToCollectionAuthorizer(this.context, false)
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    try {
      ...
      long count = Base.exec(AJEntityContent.ADD_QUESTION_QUERY, this.context.collectionId(),
      ...
      if (count == 1) {
        ...
        LOGGER.error(
            "Question '{}' add to collection '{}' failed as question is not available or non existent",
            this.context.questionId(), this.context.collectionId());
        return new ExecutionResult<>(MessageResponseFactory
            .createInternalErrorResponse(
                resourceBundle.getString("question.not.exists.or.not.available")),
            ExecutionResult.ExecutionStatus.FAILED);
      } else {
        LOGGER
            .error("Something is wrong. Adding question '{}' to collection '{}' updated '{}' rows",
                this.context.questionId(), this.context.collectionId(), count);
      }
    } catch (DBException e) {
      LOGGER.error("Not able to add question '{}' to collection '{}'", this.context.questionId(),
    ...
    return new ExecutionResult<>(
        MessageResponseFactory
            .createInternalErrorResponse(resourceBundle.getString("question.add.fail")),
        ExecutionResult.ExecutionStatus.FAILED);
  }
