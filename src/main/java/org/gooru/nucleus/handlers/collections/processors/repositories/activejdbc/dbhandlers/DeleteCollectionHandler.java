...
class DeleteCollectionHandler implements DBHandler {
    ...
    if (context.collectionId() == null || context.collectionId().isEmpty()) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory
              .createNotFoundResponse(resourceBundle.getString("collection.id.missing")),
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (!deleteContents()) {
      return new ExecutionResult<>(
          MessageResponseFactory
              .createInternalErrorResponse(resourceBundle.getString("contents.delete.error")),
          ExecutionResult.ExecutionStatus.FAILED);
    }
        ...
        return new ExecutionResult<>(
            MessageResponseFactory.createNoContentResponse(resourceBundle.getString("deleted"),
                ...
                TagAggregatorRequestBuilderFactory
                    .getLessonTagAggregatorRequestBuilder(lessonId, tagDiff)),
  ...
  private boolean deleteContents() {
    try {
      long deletedContentCount =
          Base.exec(AJEntityContent.DELETE_CONTENTS_QUERY, this.context.userId(),
              this.context.collectionId());
      long deletedRubricCount =
          Base.exec(AJEntityRubric.DELETE_RUBRICS_QUERY, this.context.userId(),
              this.context.collectionId());
      LOGGER.info("Collection '{}' deleted along with '{}' questions and '{}' rubrics",
          context.collectionId(),
