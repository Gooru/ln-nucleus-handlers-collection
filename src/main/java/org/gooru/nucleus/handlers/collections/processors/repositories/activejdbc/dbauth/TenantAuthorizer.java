...
class TenantAuthorizer implements Authorizer<AJEntityCollection> {
  ...
  public ExecutionResult<MessageResponse> authorize(AJEntityCollection model) {
    ...
    ContentTenantAuthorization authorization = ContentTenantAuthorizationBuilder
        .build(contentTenantTree, userTenantTree,
            ContentTreeAttributes.build(model.isCollectionPublished()));
  ...
  private ExecutionResult<MessageResponse> checkAuthBasedOnCourseBeingPublished(
    ...
    if (courseId != null) {
      try {
        long published =
            Base.count(AJEntityCollection.TABLE_COURSE, AJEntityCollection.PUBLISHED_FILTER,
                courseId);
        if (published >= 1) {
