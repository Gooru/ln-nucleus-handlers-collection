...
class AddResourceToCollectionHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    try {
      ...
      long count = Base.exec(AJEntityContent.ADD_RESOURCE_QUERY, this.context.collectionId(), this.collection.getString(AJEntityCollection.COURSE_ID),
        ...
        sequenceId, this.context.resourceId(), this.context.userId());
      if (count == 1) {
        ...
        LOGGER.error("Resource '{}' add to collection '{}' failed as question is not available or non existent", this.context.resourceId(),
          this.context.collectionId());
        return new ExecutionResult<>(
          MessageResponseFactory.createInternalErrorResponse(resourceBundle.getString("resource.not.exists.or.not.available")),
          ExecutionResult.ExecutionStatus.FAILED);
      } else {
