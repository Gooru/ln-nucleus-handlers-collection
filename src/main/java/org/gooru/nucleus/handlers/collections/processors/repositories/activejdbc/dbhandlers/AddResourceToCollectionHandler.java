...
class AddResourceToCollectionHandler implements DBHandler {
    ...
    public ExecutionResult<MessageResponse> checkSanity() {
        ...
        if (context.collectionId() == null || context.collectionId().isEmpty() || context.resourceId() == null
            || context.resourceId().isEmpty()) {
            LOGGER.warn("Missing collection/question id");
            return new ExecutionResult<>(
                MessageResponseFactory.createInvalidRequestResponse(resourceBundle.getString("content.id.missing")),
    ...
    public ExecutionResult<MessageResponse> validateRequest() {
        ...
        return AuthorizerBuilder.buildAddContentToCollectionAuthorizer(this.context, true).authorize(collection);
    }
    ...
    public ExecutionResult<MessageResponse> executeRequest() {
        try {
            ...
            long count = Base.exec(AJEntityContent.ADD_RESOURCE_QUERY, this.context.collectionId(),
                ...
                this.collection.getString(AJEntityCollection.LESSON_ID), this.context.userId(), sequenceId,
            ...
            if (count == 1) {
                ...
                return new ExecutionResult<>(MessageResponseFactory
                    .createNoContentResponse(resourceBundle.getString("resource.added"), EventBuilderFactory
                        .getAddContentToCollectionEventBuilder(context.collectionId(), context.resourceId())),
                ...
                return new ExecutionResult<>(MessageResponseFactory
                    .createInternalErrorResponse(resourceBundle.getString("resource.not.exists.or.not.available")),
                    ExecutionResult.ExecutionStatus.FAILED);
            } else {
                LOGGER.error("Something is wrong. Adding resource '{}' to collection '{}' updated '{}' rows",
        ...
        return new ExecutionResult<>(
            MessageResponseFactory.createInternalErrorResponse(resourceBundle.getString("resource.add.failed")),
            ExecutionResult.ExecutionStatus.FAILED);
    }
