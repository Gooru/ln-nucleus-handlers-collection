...
class ReorderContentInCollectionHandler implements DBHandler {
    ...
    public ExecutionResult<MessageResponse> validateRequest() {
        ...
        try {
            List idList =
                Base.firstColumn(AJEntityContent.CONTENT_FOR_REORDER_COLLECTION_QUERY, this.context.collectionId());
            ...
            if (idList.size() != input.size()) {
                return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(
                    resourceBundle.getString("question.count.mismatch")), ExecutionResult.ExecutionStatus.FAILED);
            }
            for (Object entry : input) {
                String payloadId = ((JsonObject) entry).getString(AJEntityCollection.ID);
                if (!idList.contains(UUID.fromString(payloadId))) {
                    return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(
                        resourceBundle.getString("missing.contents")), ExecutionResult.ExecutionStatus.FAILED);
                }
            }
        } catch (DBException | ClassCastException e) {
            return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(
                resourceBundle.getString("payload.data.types.incorrect")), ExecutionResult.ExecutionStatus.FAILED);
        }
