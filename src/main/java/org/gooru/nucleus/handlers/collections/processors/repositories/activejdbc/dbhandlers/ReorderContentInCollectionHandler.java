...
class ReorderContentInCollectionHandler implements DBHandler {
  ...
  private static final Logger LOGGER = LoggerFactory
      .getLogger(ReorderContentInCollectionHandler.class);
  ...
  public ExecutionResult<MessageResponse> checkSanity() {
    ...
    JsonObject errors = new DefaultPayloadValidator().validatePayload(context.request(),
        AJEntityCollection.reorderFieldSelector(), AJEntityCollection.getValidatorRegistry());
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    ...
    try {
      List idList =
          Base.firstColumn(AJEntityContent.CONTENT_FOR_REORDER_COLLECTION_QUERY,
      ...
      if (idList.size() != input.size()) {
        return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(
            resourceBundle.getString("question.count.mismatch")),
            ExecutionResult.ExecutionStatus.FAILED);
      }
      for (Object entry : input) {
        ...
        if (!idList.contains(UUID.fromString(payloadId))) {
          return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(
              resourceBundle.getString("missing.contents")),
              ExecutionResult.ExecutionStatus.FAILED);
        }
      }
    } catch (DBException | ClassCastException e) {
      return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(
          resourceBundle.getString("payload.data.types.incorrect")),
          ExecutionResult.ExecutionStatus.FAILED);
    }
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    try {
      ...
      if (!result) {
        LOGGER.error("Collection with id '{}' failed to save modified time stamp",
      ...
      // validation itself
      LOGGER
          .error("Not able to update the sequences for collection '{}'", context.collectionId(), e);
      return new ExecutionResult<>(
          MessageResponseFactory
    ...
    return new ExecutionResult<>(
        MessageResponseFactory.createNoContentResponse(resourceBundle.getString("updated"),
            EventBuilderFactory.getReorderContentEventBuilder(context.collectionId())),
