  ...
  private ExecutionResult<MessageResponse> authorizeForContent(AJEntityCollection collection) {
    try {
      LazyList<AJEntityContent> contents =
          AJEntityContent.where(AJEntityContent.CONTENT_FOR_ADD_FILTER, context.questionId(),
              context.userId());
      if (contents.size() == 1) {
        if (isContentResource) {
          if (contents.get(0).isContentOriginal()) {
            LOGGER.warn(
                "Resource '{}' being added to collection '{}' is not a reference but original",
                context.resourceId(), context.collectionId());
            return new ExecutionResult<>(MessageResponseFactory
                .createInvalidRequestResponse(
                    resourceBundle.getString("resource.reference.needed")),
                ExecutionResult.ExecutionStatus.FAILED);
          }
        }
      ...
      LOGGER.error("Error querying content '{}' availability for associating in collection '{}'",
          context.questionId() != null ? context.questionId() : context.resourceId(),
          context.collectionId(), e);
      return new ExecutionResult<>(MessageResponseFactory
          .createInternalErrorResponse(
              resourceBundle.getString("internal.error.authorization.checking")),
          ExecutionResult.ExecutionStatus.FAILED);
    ...
    return new ExecutionResult<>(MessageResponseFactory
        .createInvalidRequestResponse(
            resourceBundle.getString("content.association.not.available")),
        ExecutionResult.ExecutionStatus.FAILED);
  }
